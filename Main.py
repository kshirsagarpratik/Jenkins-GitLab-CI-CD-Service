...
r = requests.get('https://api.github.com/search/repositories?q=language:java+topic:maven', auth = (,))
#r.json()
for item in r.json()['items']:
	...
	g = requests.post('http://localhost:30080/api/v4/projects', data = {'name':repo_name,'import_url':repo_url}, headers = {'Private-Token':''})
...
server = jenkins.Jenkins('http://localhost:8080/', username='', password='') # Pass Jenkins host URL and credentials.
user = server.get_whoami()
#version = server.get_version()     # this method is bugged.
print('Hello %s from Jenkins' % (user['fullName']))

gl = gitlab.Gitlab('http://localhost:30080', private_token='') # Pass the gitlab host URL and generated private API token (from gitlab.)
gl.auth()
